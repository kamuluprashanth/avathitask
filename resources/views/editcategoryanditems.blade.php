...
<script type="text/javascript">
     var selectitem = null;
      ...
      if ({{count($categorys)}} > 0) {
        selectitem = <?php echo json_encode($categorys );?>;
        
      }
   
    console.log(selectitem);
    var selecteditems,selectedvalue;
    $.each(selectitem,function(key,value){
selecteditems = value.catalog_id;
console.log(selecteditems);
selectedvalue = value.cid;
$('#txtid').val(selectedvalue);
$('#selectitem option[value=' + selecteditems + ']').attr('selected', true);
    });
                        ...
                        <form class="form-horizontal" role="form" method="POST" action="{{ url('admin/storecategroyitems') }}">
                            ...
                            <div class="form-group{{ $errors->has('selectcategory') ? ' has-error' : '' }}">
                                <label for="selectcategory" class=" control-label">Category:</label>
                                    ...
                                    <select id="selectcategory" type="text" class="form-control js-example-basic-multiple" name="selectcategory" value="{{ old('selectcategory') }}"
                                           required>
                                               @if(!empty($category))
                            ...
                            <div class="form-group{{ $errors->has('selectitem') ? ' has-error' : '' }}">
                                <label for="password" class=" control-label">Items:</label>
                                    ...
                                    <select id="selectitem" type="text" class="form-control js-example-basic-multiple" name="selectitem[]" value="{{ old('selectitem') }}"
